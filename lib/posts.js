...
export function getPublicPostSlugs() {
...
export function getGraphData() {
    ...
    const nodes = posts.map((post) => ({
        ...
        group: post.group || post.category || 'Uncategorized',
        series: post.series || '',
        hub: parseBooleanFrontmatter(post.hub),
        date: post.date || '',
    }));
    ...
    posts.forEach((post) => {
        post.links.forEach((link) => {
            const targetSlug = normalizePostSlug(link.target);
            if (slugMap.has(targetSlug)) {
                ...
                if (!linkSet.has(key)) {
                    ...
                    links.push({
                        source: post.slug,
                        target: targetSlug,
                        series: getSharedSeries(post, slugMap.get(targetSlug)),
                    });
                }
            }
        });
...
function isVisiblePost(post) {
    ...
    return shouldShowDrafts() || !parseBooleanFrontmatter(post.draft);
}
...
function getSharedSeries(sourcePost, targetPost) {
    if (!sourcePost?.series || !targetPost?.series) return '';
    return sourcePost.series === targetPost.series ? sourcePost.series : '';
}
