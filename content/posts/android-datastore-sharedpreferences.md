...
## 암호문을 복호화하는 키는 keyset일까

여기서 한 번 더 헷갈릴 수 있다.

```text
DataStore에 저장된 암호문을 다시 열 때 쓰는 키가 keyset인가?
```

큰 방향에서는 맞다. 다만 조금 더 정확히 말하면, 앱 코드가 keyset 안의 raw key를 직접 꺼내서 `decrypt()`에 넣는 구조는 아니다.

흐름은 이렇게 보는 편이 정확하다.

```text
Tink keyset
-> Tink가 AEAD primitive를 만든다.
...
즉 DataStore의 암호문을 복호화할 때 필요한 암호화 키는 Tink keyset 안에 있다. 하지만 앱은 보통 그 키 원본을 직접 만지지 않는다. 앱은 Tink가 만들어준 `Aead` 객체에 암호문과 associated data를 넘기고, Tink가 keyset을 바탕으로 복호화를 수행한다.
...
이 코드에서 `aead`는 그냥 함수 묶음처럼 보이지만, 내부적으로는 Tink keyset을 기반으로 만들어진 암호화 primitive다.

그래서 관계를 한 줄씩 나누면 이렇다.
...
그럼 다음 문제가 생긴다. keyset 안에 암호문을 열 수 있는 키가 있다면, keyset 자체도 민감한 데이터다.

```text
DataStore의 암호문만 노출
-> keyset이 없으면 열기 어렵다.

DataStore의 암호문 + keyset 원문 노출
-> 암호문을 열 수 있는 위험이 커진다.
```

그래서 keyset도 평문으로 아무 파일에 두면 안 된다. Android에서는 이 keyset을 Android Keystore의 master key로 보호하는 구성을 사용할 수 있다.

## master key는 무엇을 여는 키일까

`master key`라는 말도 이름 때문에 오해하기 쉽다. 마치 이 키가 DataStore에 저장된 개인키 암호문을 직접 복호화하는 최종 열쇠처럼 들릴 수 있다.

하지만 Tink와 Keystore를 함께 쓰는 구조에서는 보통 이렇게 보는 게 더 정확하다.
...
즉 master key는 개인키 같은 원문 데이터를 직접 암호화하는 키라기보다, **그 데이터를 암호화하는 데 쓰는 keyset을 다시 보호하는 키**에 가깝다.
...
여기서 중요한 점은 앱이 master key 원문을 직접 꺼내오지 않는다는 것이다. 앱은 보통 alias나 Keystore URI로 "이 Keystore 키를 사용해 달라"고 요청한다.
...
Tink keyset 보호용 master key로 사용한다.
```

앱이 하는 일은 alias를 지정하는 것이다. master key의 실제 바이트를 받아서 변수에 담거나 파일에 저장하는 구조가 아니다.
...
Android Keystore
-> master key를 내부에서 사용
-> keyset 보호/해제에 필요한 작업 수행

앱
-> master key 원문은 받지 않음
```

그래서 전체 구조는 이렇게 정리할 수 있다.
...
Android Keystore master key
-> 앱이 원문을 직접 받지 않고 alias로 사용
```

## 전체 흐름 그림으로 정리하기

글로 풀면 길지만, 그림으로 보면 책임이 나뉜다.

![DataStore, Tink keyset, Android Keystore 관계 흐름도](/images/android-datastore-keyset-masterkey-flow-handdrawn.png)

저장할 때는 원문이 Tink AEAD를 거쳐 암호문이 되고, 그 암호문이 DataStore에 남는다.

읽을 때는 DataStore에서 암호문을 꺼내고, Tink AEAD가 keyset을 바탕으로 복호화한다.

그리고 keyset 자체는 다시 Android Keystore의 master key로 보호된다. 그래서 DataStore는 저장소, Tink는 암호화 도구, Keystore는 keyset을 보호하는 보안 계층으로 나뉜다.
...
DataStore 암호문을 복호화하는 키는 keyset인가?
-> 큰 방향에서는 맞다. 더 정확히는 Tink keyset을 기반으로 만든 AEAD가 복호화를 수행한다.

master key는 DataStore 암호문을 직접 여는 키인가?
-> 보통은 아니다. Tink keyset을 보호하는 상위 키로 이해하는 편이 정확하다.
```
...
그래서 로컬에 민감값을 저장한다면 DataStore만 보는 것이 아니라, [[tink-aead-basics-android-wallet|Tink AEAD]]와 [[android-keystore-basics|Android Keystore]]까지 함께 봐야 한다. 저장소에는 암호문만 남기고, 암호화 키는 별도의 보안 계층에서 보호하는 식으로 책임을 나눠야 구조가 선명해진다.
