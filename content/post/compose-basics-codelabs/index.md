...
## 11. 목록에 애니메이션 적용
![](https://developer.android.com/static/codelabs/jetpack-compose-basics/img/a5267f0b704bf355.gif?hl=ko)

이를 위해 animateDpAsState 컴포저블을 사용한다. 이 컴포저블은 애니메이션이 완료될 때까지 애니메이션에 의해 객체의 value 가 계속 업데이트되는 상태 객체를 반환한다. 유형이 Dp인 '목표 값'을 사용
```kotlin
...
(animate*AsState) 를 사용하여 만든 애니메이션은 중단될 수 있다. 즉, 애니메이션 중간에 목표 값이 변경되면 (animate*AsState)는 애니메이션을 다시 시작하고 새 값을 가리킨다. 

특히 스프링 기반 애니메이션에서는 중단이 자연스럽게 보인다.

![](https://developer.android.com/static/codelabs/jetpack-compose-basics/img/e7d244875b3b6b6f.gif?hl=ko)

## 12. 앱의 스타일 지정 및 테마 설정

ui.theme.kt 에 BasicsCodelabTheme 이 있고 그안에 MaterialTheme 가 선언되어있다.
...
BasicsCodelabTheme 은 MaterialTheme 을 내부적으로 래핑하므로 MyApp 은 테마에 정의된 속성으로 스타일이 지정된다. 

모든 하위 컴포저블에서 MaterialTheme 의 세 가지 속성, colorScheme, typography, shapes 를 가져올 수 있다

```kotlin
Text(text = name, style = MaterialTheme.typography.headlineMedium)
```

위의 예에서 Text 컴포저블은 새 TextStyle을 설정한다. 고유한 TextStyle 을 만들 수도 있고 기본인 MaterialTheme.typography 를 사용하여 테마가 정의된 스타일을 가져올 수도 있다.

이 구성을 사용하면 Material 에 정의된 텍스트 스타일(displayLarge, headlineMedium, titleSmall, bodyLarge, labelMedium 등)에 액세스할 수 있다. 

이 예제에서는 테마에 정의된 headlineMedium 스타일을 사용한다.

그리고 copy 함수를 사용하여 미리 정의된 스타일을 수정할 수 있다. 아래는 숫자를 더 굵게 만드는 예시
...
![숫자를 표시하는 Text 컴포저블에 ExtraBold 를 지정](https://developer.android.com/static/codelabs/jetpack-compose-basics/img/fc2839dd49627cd1_1920.png?hl=ko)


### 어두운 모드 미리보기 설정
DefaultPreview 에 UI_MODE_NIGHT_YES 와 함께 @Preview 주석을 추가하면
두개의 프리뷰를 표시할 수 있다.
...
![](https://developer.android.com/static/codelabs/jetpack-compose-basics/img/3b6e25f5ee00a2e2_1920.png?hl=ko)

### 앱 테마 조정

ui/theme 폴더에 있는 파일에서 현재 테마와 관련된 모든 항목을 찾을 수 있다. 예를 들어, 지금까지 사용한 기본 색상은 Color.kt 에 정의되어 있다.

Color.kt 에 다음과 같이 새로운 색상을 추가 할 수 있다.
```kotlin
...
이제 Theme.kt 에서 MaterialTheme 팔레트에 색상을 할당한다.
```kotlin
...
MainActivity.kt 로 돌아가서 미리보기를 새로고침해도 미리보기 색상은 실제로 변경되지 않는데 이부분은 미리보기에서 기본적으로 동적 색상이 사용되기 때문에
동적 색상은 아래 링크를 참조하자.

동적 색상: https://m3.material.io/styles/color/dynamic-color/overview

색 구성표의 비 적응형 버전을 보려면 API 수준이 31(적응형 색이 도입된 Android S 에 해당)보다 낮은 기기에서 앱을 실행하면된다. 
다음과 같은 새로운 색상이 표시되는것을 확인할 수 있다

![31 미만에서 실행](https://developer.android.com/static/codelabs/jetpack-compose-basics/img/493d754584574e91_1920.png?hl=ko)

이번 머터리얼3에 대한 구글 IO 를 좀 챙겨봐야할 것 같다

코드랩 최종 단계에서는 버튼에 이미지를 추가하는데 전부 완료하였다.

추후 아래의 코드랩도 진행해보면서 컴포즈에 대해서 하나씩 공부해야겠다.


### 다음 단계
아래 Compose 개발자 과정에서 다른 Codelab 을 확인
https://developer.android.com/courses/pathways/compose?hl=ko

- [Compose 레이아웃](https://developer.android.com/codelabs/jetpack-compose-layouts?hl=ko#0)
- [Compose에서의 상태](https://developer.android.com/codelabs/jetpack-compose-state?hl=ko#0)
- [Compose 테마 설정](https://developer.android.com/codelabs/jetpack-compose-theming?hl=ko#0)
- [기존 앱을 위한 Compose](https://developer.android.com/codelabs/jetpack-compose-migration?hl=ko#0)
...
### Compose 애니메이션 관련 공식문서
- 링크: https://developer.android.com/jetpack/compose/animation?hl=ko